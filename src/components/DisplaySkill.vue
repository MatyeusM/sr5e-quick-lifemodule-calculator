...
<template>
  ...
  <span class="relative">
    ...
    <div v-if="isDropdownOpen" class="z-50 left-0 p-2 w-60 origin-top-left border border-white absolute bg-slate-900" style="margin-top: -30px;">
      ...
      <div v-if="IsGroup || HasGroup" @wheel="handleWheel(Group, GroupRating, $event)" :class="{ 'text-lg text-amber-200 border-b border-amber-200': IsGroup, 'border-b border-slate-100': !IsGroup }">
        {{ Group }} {{ GroupRating }}
      </div>
